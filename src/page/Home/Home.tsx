  ...
  return (
    <Container>
     <div className='centralizar'>  
      <div >
        <div></div>
        {exibirImagem && <div className='chat'><img src={Loading} width="50px" alt="loading" /></div>}
        {!exibirImagem && (
          <>
            <div className='chat'>
    ...
    {exibirImagem3 && <div></div> }{!exibirImagem3 && (          
          <div>
            {exibirImagem4 && <div className='chat'><img src={Loading} width="50px" alt="loading" /></div>}
            {!exibirImagem4 && (
              <div>
                <button onClick={() => setPrimeiroNomeChecked(!primeiroNomeChecked)}>
                  {primeiroNome}
                </button>
                <button onClick={() => setNomeDaMaeChecked(!nomeDaMaeChecked)}>
                  {result.mae}
                </button>
                <button onClick={() => setSegundoNomeChecked(!segundoNomeChecked)}>
                  {segundoNome}
                </button>
              </div>
            )}
